...
from .transformers import (
    transform_invalid_ref_to_none,
    transform_na_to_none,
    transform_tags_to_tags,
)
...
class Field(object):
    ...
    def __init__(
        ...
        index: int = -1,
        allow_empty_or_null: bool = False,
        default_val: Any = None,
        pre_transform: Optional[List[Callable]] = None,
        post_transform: Optional[List[Callable]] = None,
        **others,
    ):
        ...
        self._meta = dict(
            {
                "name": name,
                "index": index,
                "allow_empty_or_null": allow_empty_or_null,
                "default_val": default_val,
                "pre_transform": [transform_invalid_ref_to_none, transform_na_to_none]
                + pre_transform,
    ...
    def validate(self, headers):
        ...
        if index in range(len(headers)):
            ...
            field_checks.append(
                f"index attribute was out of range. [given: {index}], [header_size: {len(headers)}]"
            )
    ...
    def __repr__(self):
        ...
        for field_name, field_itself in self._meta.items():
            field_value = (
                field_itself.__name__
                if type(field_itself).__name__ == "type"
                else field_itself
            )
    ...
    def _to_decimal(self, val):
        ...
        try:
            ...
            raise FieldException(
                f"Not all the items are of same type: {type_of}, value {val}"
            )
        else:
            return val
